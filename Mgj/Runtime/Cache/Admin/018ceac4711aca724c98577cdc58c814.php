                        ...
                        <div class="col-xs-12">
                            <div class="col-xs-12">
                                <div class="" id="nav-search" style="margin-bottom: 10px">
                                    <form class="form-search" action="<?php echo U('Visitor/importantpeo');?>" method="get">
                                        <input type="text" placeholder="学校名称" name="vs_ocodename" class="nav-search-input"  autocomplete="off" value="<?php echo ($cond["vs_ocodename"]); ?>"/>
                                        <input type="text" placeholder="姓名" name="vs_username" class="nav-search-input"  autocomplete="off" value="<?php echo ($cond["vs_username"]); ?>"/>
                                        开始时间:
                                        <input type="text"  class="nav-search-input" id="begintime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="starttime" placeholder="" value="<?php echo ($cond["starttime"]); ?>">
                                        结束时间:
                                        <input type="text"  class="nav-search-input" id="endtime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="endtime" placeholder="" value="<?php echo ($cond["endtime"]); ?>">
                                ...
                                <form action="" method="post">
                                    <table id="sample-table-1" class="table table-striped table-bordered table-hover" style="text-align:center;">
                                        ...
                                        <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
                                                <td class="center"><?php echo ($info["rec_id"]); ?></td>
                                                <td class="center"><?php echo ($info["hosid"]); ?></td>
                                                <td class="center"><?php echo ($info["cardid"]); ?></td>
                                                <td class="center"><?php echo ($info["sususername"]); ?></td>
                                                <td class="center"><?php echo ($info["sususertel"]); ?></td>
                                                <td class=""><?php echo ($info["susevent"]); ?></td>
                                                <td class="center"><?php echo ($info["sustime"]); ?></td>
                                                <td class="center"><?php echo ($info["sustype"]); ?></td>
                                                <td class="center"><?php echo ($info["susstate"]); ?></td>
                                                <td class="center"><?php echo ($info["regtime"]); ?></td>
                                                <td class="center"><button type="button" class="btn btn-warning btn-xs delete_visitor" ab="visitorssususer" rec_id="<?php echo ($info["rec_id"]); ?>">删除</button></td>
                                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                                        <tr>
                                            <td colspan="12" id="pages"><div class="row"><div class="col-md-13"></div><?php echo ($page); ?></td>
                                        </tr>

                                        </tbody>
                                    </table>
                                </form>
                                <button type="button" class="btn btn-info btn-sm addimport">添加</button>
    ...
    <div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" id="addGrowmodel">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <form action="addimportantpeo" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">添加重点人员</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="cardid">证件号:</label>
                            <input type="text" name="CardId" class="form-control" id="cardid" placeholder="">
                            <label for="sususername">姓名:</label>
                            <input type="text" name="SuSuserName" class="form-control" id="sususername" placeholder="">
                            <label for="SuSuserTel">电话:</label>
                            <input type="text" name="SuSuserTel" class="form-control" id="sususertel" placeholder="">
                            <label for="SuSEvent">事件:</label>
                            <input type="text" name="SuSEvent" class="form-control" id="susevent" placeholder="">
                        </div>
                    </div>
