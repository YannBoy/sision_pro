...
<html lang="en">
    ...
    <div class="main-content">
                ...
                <div class="col-xs-12">
                    ...
                    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                        <tr>
                            ...
                            <td >
                                <input type="text" id="title" class="col-md-6" value="<?php echo ($data["title"]); ?>">
                            </td>
                        </tr>
                        <tr>
                            <td style="text-align:right;" class="col-md-2">发布日期：</td>
                            <td>
                                <input size="20" type="text" id="regtime"  readonly class="form_datetime" value="<?php echo ($data["do_time"]); ?>">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <div style="width:500px;height:100px;border:1px solid #999;overflow:auto;" class="teacher_all_name">
                                    <?php if($data['all_people'] == 1): ?><button class="btn btn-info btn-xs" name="teacher_name" value="0">全体教师</button>
                                        <?php else: ?>
                                        <?php if(is_array($peo)): $i = 0; $__LIST__ = $peo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$peo): $mod = ($i % 2 );++$i;?><button type="button" class="btn btn btn-info btn-xs"><?php echo ($peo["name"]); ?></button> &nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; endif; ?>
                                </div>
                                <br/>
                        ...
                        <tr>
                            <td colspan="2">
                                <form action="Index/add" method="post">
                                    <script id="container" name="content" type="text/plain" style="margin:0 auto;width:900px;height:500px"><?php echo ($data["content_info"]); ?></script>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="file_dis">
                                    <?php if(is_array($file)): $i = 0; $__LIST__ = $file;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><span class="col-lg-5"><?php echo ($vo["name"]); ?></span>&nbsp;<button name="oa_file_del" id="<?php echo ($vo["id"]); ?>" title="notice_upload_file">删除</button>
                    ...
                    <button class="btn btn-info" type="button"  name="notice_add">保存</button>
                </div><!-- /.col -->
            </div><!-- /.row -->
        ...
        <script src="/Public/Admin/js/jquery-1.8.3.min.js"></script>
        <script type="text/javascript" src="/Public/Home/js/ueditor.config.js"></script>
        <!-- 编辑器源码文件 -->
        <script type="text/javascript" src="/Public/Home/js/ueditor.all.js"></script>
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                $('#btn_upload').uploadify({
                    uploader: "http://"+url+"/index.php/Comment/notice_upload_file",            // 服务器处理地址
                    ...
                    onUploadSuccess: function (file, data, response) {   //一个文件上传成功后的响应事件处理
                        ...
                        var file_id = $('#id').val();
                        $.ajax({
                            url:"http://"+url+"/index.php/Comment/notice_file",
                            type:'POST',
                            ...
                            success:function(data){
                                if(data[0].success == true){
                                    ...
                                    for($i=0;$i<data[0].data.length;$i++){
                                        html = '<span class="col-lg-5">'+data[0].data[$i].name+'</span> <button id='+data[0].data[$i].id+' name="oa_file_del" >删除</button> </br>'
