...
<html lang="en">
    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
                ...
                <li class="active">已授权用户列表</li>
            </ul><!-- .breadcrumb -->

            <div class="nav-search" id="nav-search">
                <form class="form-search" action="<?php echo U('Admin/User/group_user');?>" method="get">
                    <span>
                        <label>
                    ...
                    <span class="input-icon">
                        <input type="text" placeholder="用户姓名 ..." name="keyword" class="nav-search-input" id="nav-search-input" autocomplete="off" value="<?php echo ($keyword); ?>"/>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="table-responsive">
                                        ...
                                        <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                                                ...
                                                <td>
                                                    <div class="center" class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                        <?php if($v["vidflag"] == 0): ?><button type="button" class="btn btn-success btn-xs teacher_vidf" name="<?php echo ($v["userid"]); ?>" id="user" value="1">停用</button>
                                                            <?php else: ?> <button type="button" class="btn btn-success btn-xs teacher_vidf" name="<?php echo ($v["userid"]); ?>" id="user" value="0">启用</button><?php endif; ?>
                                                        <button type="button" class="btn btn-info btn-xs get_id" data-toggle="modal" data-target="#myModal" name="<?php echo ($v["userid"]); ?>">
                                                            身份管理
                                                        </button>
                                                        <a href="/index.php/User/user_edit/id/<?php echo ($v["userid"]); ?>.html" class="btn btn-xs btn-info">
                                                            <i class="icon-edit bigger-120"></i>
                                                        </a>

                                                        <a href="/index.php/User/delete/id/<?php echo ($v["userid"]); ?>.html" class="btn btn-xs btn-warning"/>
                                ...
                                <!--<a href="<?php echo U('admin_adduser');?>"><input type="submit" class="btn btn-primary btn-xs"  value="添加用户"></a>-->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="height:500px">
                ...
                <div class="modal-body" style="overflow:scroll;height:400px">
                    <table class="table table-hover" name="">
                        <tr>
                            <th class="center">序号</th>
                            <th class="center">身份名称</th>
                            <th class="center">操作</th>
                        </tr>
                        <tbody class="identity_info">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default bake" data-toggle="modal" data-target=".bs-example-modal-sm" name="">添加新身份</button>
    ...
    <div class="modal fade bs-example-modal-sm"  role="dialog" aria-labelledby="mySmallModalLabel" style="margin-top:80px">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" >添加新身份</h4>
                </div>
                <div style="padding:30px">
                    <input type="text" class="form-control identity" id="" placeholder="请输入新身份">
                </div>
                <div class="modal-footer" style="height:50px">
                    <button type="button" class="btn btn-primary btn-xs add_shenfen" data-dismiss="modal" aria-label="Close" id="add" name="">确定</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('.get_id').click(function(){
            $('.bake').attr('name',$(this).attr('name'));
            var id = $(this).attr('name');
            $('.identity_info').html('');
            $.ajax({
                url:"http://"+url+"/index.php/Comment/get_identity",
                type:'POST',
                data:{id:id},
                dataType:'json',
                success:function(data){
                    ...
                    if(data[0].success == true)
                    {
                        for($i = 0; $i<data[0].data.length;$i++){
                            $j = $i+1;
                            tr = "<tr class="+data[0].data[$i].id+"> <td class='center' > "+ $j + "</td> <td class='center'>"+data[0].data[$i].identity+"</td> <td class='center'><button class='btn btn-warning btn-xs identity_del' id='del'  name="+data[0].data[$i].id+" >删除</button></td> </tr>";
                            $('.identity_info').append(tr);
                        }
                    }else{
                        alert(data[0].msg);
                    }
        ...
        $('.bake').click(function(){
            $('.add_shenfen').attr('name',$(this).attr('name'));
        });

        $('.add_shenfen').click(function(){
            ...
            $('.identity_info').html('');
            $.ajax({
                url:"http://"+url+"/index.php/User/identity",
                type:'POST',
                data:{id:id,identity:identity,desc:desc},
                dataType:'json',
                success:function(data){
                    if(data.info == '没有权限'){
                        alert(data.info);
        ...
        $(document).on("click",".identity_del",function(){
            var desc = $(this).attr('id');
            var id = $(this).attr('name');
            $.ajax({
                url:"http://"+url+"/index.php/User/identity",
