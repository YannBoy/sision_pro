...
$('button[name=make_notice_id1]').click(function(){
    ...
    var id = $(this).attr('id');
    $.ajax({
        url: "http://" + url + "/index.php/Comment/del_email",
        type: 'POST',
        data: {id: id},
        dataType: 'json',
        success: function (data) {
...
$('button[name=make_notice_id2]').click(function(){
    var url = $('#url').val();
    var id = $(this).attr('id');
    $.ajax({
            ...
            if(data[0].success == true){
                for($i=0;$i<data[0].data.length;$i++){
                    if(data[0].data[$i].vidflag == 0){
                       //$('#num_id2').append("<option value="+data[0].data[$i].area_bh+">"+data[0].data[$i].area_name+"</option>");
                      tr = "<input type='checkbox' id="+data[0].data[$i].rec_id+" class='true_peo'/> <label style='cursor:pointer' for="+data[0].data[$i].rec_id+">"+data[0].data[$i].stu_name+"</label> &nbsp;";
    ...
    $.ajax({
        ...
        success: function(data){
            if(data[0].success == true){
                for($i=0;$i<data[0].data.length;$i++){
                        //$('#num_id2').append("<option value="+data[0].data[$i].area_bh+">"+data[0].data[$i].area_name+"</option>");
                        tr = "<input type='checkbox'  id="+data[0].data[$i].rec_id+" class='true_peo1'/> <label style='cursor:pointer' for="+data[0].data[$i].rec_id+">"+data[0].data[$i].class_name+"</label> &nbsp;";
    ...
    $.ajax({
        ...
        success: function(data){
            if(data[0].success == true){
                for($i=0;$i<data[0].data.length;$i++){
                    //$('#num_id2').append("<option value="+data[0].data[$i].area_bh+">"+data[0].data[$i].area_name+"</option>");
                    tr = "<input type='checkbox' id="+data[0].data[$i].tel+" class='true_peo2' value="+data[0].data[$i].name+"  /> <label style='cursor:pointer' for="+data[0].data[$i].tel+">"+data[0].data[$i].name+"</label> &nbsp;";
    ...
    $.ajax({
        ...
        success: function(data){
            if(data[0].success == true){
                for($i=0;$i<data[0].data.length;$i++){
                    //$('#num_id2').append("<option value="+data[0].data[$i].area_bh+">"+data[0].data[$i].area_name+"</option>");
                    tr = "<input type='checkbox' id="+data[0].data[$i].id+" class='true_peo3' value="+data[0].data[$i].name+"  /> <label style='cursor:pointer' for="+data[0].data[$i].id+">"+data[0].data[$i].name+"</label> &nbsp;";
...
$(document).mousemove(function(e) {
    ...
    if(xx < 200 || yy < 90){
        $('.tool_border').css('display','none');
    }
    var xx = xx - 550;
    var yy = yy - 148;
    $('.tool_border').css({top: yy, left: xx});

});
$('div[name=remove_tool]').dblclick(function () {
    $('.tool_border').css('display','block');
});

$('div[name=remove_tool]').click(function () {
    $('.tool_border').css('display','none');
});
